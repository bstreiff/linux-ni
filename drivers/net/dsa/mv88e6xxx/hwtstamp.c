					   ...
					   struct sk_buff_head *rxq)
{
	...
	for ( ; skb; skb = __skb_dequeue(&received)) {
		...
		if (hdr) {
			 ...
			 * into the messageTypeSpecific field in the incoming
			 * packet.
			 */
			ns = __be32_to_cpu(hdr->reserved2);

			/* The hardware, however, did not update the checksum,
			 * so reset the field to 0 so that it matches the state
			 * before hardware wrote over it.
			 */
			hdr->reserved2 = 0;
